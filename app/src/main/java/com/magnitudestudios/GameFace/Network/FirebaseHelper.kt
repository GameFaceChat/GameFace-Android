...
import com.magnitudestudios.GameFace.pojo.SessionDetails
...
class FirebaseHelper {
    ...
    private fun createRoom(callback: RoomCallback) {
        initiator = true
        sendMessage(Constants.JOINED_KEY, username).addOnCompleteListener { task: Task<Void?>? ->
    ...
    fun getUsername() {
        ...
        referenceExists(ReferenceExists { b, data ->
            if (b) {
                this.username = data.getValue() as String
            }
            else {
                this.username = "RANDOM_USER"+Math.random()*1000
            }
